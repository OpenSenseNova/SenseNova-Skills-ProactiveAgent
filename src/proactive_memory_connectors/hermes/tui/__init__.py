"""Hermes observer bridge for the Proactive Memory TUI Connector."""
...
def _is_tui(kwargs: Mapping[str, Any]) -> bool:
    ...
    requested = os.environ.get("PROACTIVE_MEMORY_HERMES_PLATFORMS", "tui")
...
def _timeout() -> float:
    try:
        return max(
            0.2,
            min(3.0, float(os.environ.get("PROACTIVE_MEMORY_HTTP_TIMEOUT", "1.5"))),
        )
    except ValueError:
        return 1.5
...
def _debug(message: str) -> None:
    if os.environ.get("PROACTIVE_MEMORY_DEBUG") == "1":
        logger.warning("proactive-memory: %s", message)
