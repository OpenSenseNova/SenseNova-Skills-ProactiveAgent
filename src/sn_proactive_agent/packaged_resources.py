...
``src/sn_proactive_agent_connectors``.  A wheel exposes that directory under
the stable ``sn_proactive_agent_connectors`` package namespace instead.  This
...
CONNECTOR_PACKAGE = "sn_proactive_agent_connectors"
...
def _checkout_connector_root() -> Path:
    # .../src/sn_proactive_agent/packaged_resources.py
    root = Path(__file__).resolve().parents[1] / "sn_proactive_agent_connectors"
    if not root.is_dir():
