...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="sn-proactive-agent")
    parser.add_argument(
        "--version",
        action="version",
    ...
    setup.add_argument(
        "--hermes-executable",
        default=env_value("SN_PROACTIVE_AGENT_HERMES") or shutil.which("hermes"),
    ...
    uninstall = subcommands.add_parser(
        "uninstall",
        help="移除 Harness Connector（保留 ~/.sn-proactive-agent 用户数据）",
    )
...
def _add_serve_arguments(serve: argparse.ArgumentParser) -> None:
    serve.add_argument(
        "--host",
        default=env_value("SN_PROACTIVE_AGENT_HOST", "127.0.0.1"),
    )
    serve.add_argument(
        "--port",
        type=int,
        default=int(env_value("SN_PROACTIVE_AGENT_PORT", "8080")),
    )
    serve.add_argument(
        "--data-root",
        type=Path,
        ...
        help=(
            "状态数据根目录（默认：~/.sn-proactive-agent；也可用 "
            "SN_PROACTIVE_AGENT_DATA_ROOT 覆盖；自动沿用已有的 ~/.proactive-memory）"
        ),
    )
    serve.add_argument(
        "--hermes",
    ...
    serve.add_argument(
        "--hermes-provider",
        default=env_value("SN_PROACTIVE_AGENT_HERMES_PROVIDER"),
    )
    serve.add_argument(
        "--hermes-model",
...
def _serve(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        with make_server(
            ...
            print(f"Proactive Agent listening on {service_url}", flush=True)
        ...
        print("\nProactive Agent stopping …", flush=True)
    finally:
