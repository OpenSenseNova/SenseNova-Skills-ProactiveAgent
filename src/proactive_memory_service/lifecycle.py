"""Installation and local lifecycle helpers for the Proactive Memory CLI.
...
``~/.proactive-memory`` and is intentionally outside the installation paths.
"""
...
def default_data_root() -> Path:
    ...
    configured = os.environ.get("PROACTIVE_MEMORY_DATA_ROOT", "").strip()
    if configured:
        return Path(configured).expanduser()
    return Path.home() / ".proactive-memory"
...
def _observer_transaction(home: Path):
    ...
    resources = {
        home / "skills/productivity/proactive-memory/connectors/hermes/classic/hermes_hook.py": "hermes/classic/hermes_hook.py",
            ...
            with _observer_transaction(home):
                ...
                with install_bridge(Path(hermes_root), home, dry_run=dry_run) as base:
                    ...
                    if dry_run:
                        ...
                        _atomic_write(home / "plugins/proactive-memory-tui/bridge-install.json", json.dumps({
...
def _setup_harness_from_resources(
    ...
    *,
    ...
    plugin_destination = home / "plugins" / "proactive-memory-tui"
    ...
    if dry_run:
        ...
        if migrated_legacy:
            ...
            print("Config: 将添加 Proactive Memory Hook", file=out)
        else:
            print("Config: 已包含 Proactive Memory Hook，无需修改", file=out)
        return SetupResult(
            "hermes",
            home,
            hook_destination,
    ...
    if install_tui_plugin:
        ...
        if executable:
            ...
            warnings.append(
                "TUI 插件已复制，但找不到 hermes；请手动运行 `hermes plugins enable proactive-memory-tui`."
            )
...
def uninstall_harness(
    ...
    *,
    ...
    out = output or sys.stdout
    home = Path(hermes_home or os.environ.get("HERMES_HOME") or (Path.home() / ".hermes")).expanduser().resolve()
    bridge_pointer = home / "plugins/proactive-memory-tui/bridge-install.json"
    ...
    plugin_path = home / "plugins" / "proactive-memory-tui"
    ...
    if dry_run:
        ...
        if config_changed:
            print(f"- 配置中的 Proactive Memory Hook: {config_path}", file=out)
...
def _remove_empty_hook_keys(lines: list[str]) -> tuple[list[str], bool]:
    """Drop Proactive Memory hook keys left empty after command removal.
...
def _backup_path(path: Path) -> Path:
    ...
    candidate = path.with_name(f"{path.name}.proactive-memory.{timestamp}.bak")
    suffix = 1
    while candidate.exists():
        candidate = path.with_name(
            f"{path.name}.proactive-memory.{timestamp}.{suffix}.bak"
        )
        suffix += 1
    return candidate
...
def _enable_tui_plugin(executable: str, hermes_home: Path) -> str | None:
    ...
    try:
        result = subprocess.run(
            [executable, "plugins", "enable", "proactive-memory-tui"],
            env=environment,
            capture_output=True,
            text=True,
            check=False,
        ...
        return f"TUI 插件已安装但无法自动启用：{exc}。请手动运行 `hermes plugins enable proactive-memory-tui`."
