...
TARGETS = ("src/app/useMainApp.ts", "src/app/proactiveMemoryWeb.ts", "dist/entry.js")
STATE_DIR = ".proactive-memory-bridge"
...
HOOK = """  // Proactive Memory: Web owns decisions; this only submits approved input.
  useEffect(() => {
    const sessionId = ui.sid
    if (!sessionId) return
    return startWebMemoryBridge({
      sessionId,
      ...
      reportError: text => sys(`Proactive Memory: ${text}`)
    })
...
def _base(root: Path) -> Path:
    ...
    for relative in (*TARGETS, "scripts/build.mjs", STATE_DIR):
        path = base / relative
...
def _write(path: Path, data: bytes) -> None:
    ...
    descriptor, name = tempfile.mkstemp(prefix=".pms-", dir=path.parent)
    temporary = Path(name)
    try:
...
def install_bridge(
    ...
    text = text.replace(IMPORT_ANCHOR, IMPORT_ANCHOR + "import { startWebMemoryBridge, submitApprovedInput } from './proactiveMemoryWeb.js'\n")
    ...
    if record_dir.exists():
        ...
        archive = Path(tempfile.mkdtemp(prefix=".pms-backup-", dir=base))
        archive.rmdir()
