...
class HermesCliConnector:
    ...
    def _respond_prefix(self) -> str:
        ...
        should use the generated ``proactive-memory-service`` executable.  In
        ...
        if (source_root / "proactive_memory_connectors").is_dir():
            return (
                f"PYTHONPATH={shlex.quote(str(source_root))} "
                "python3 -m proactive_memory_service"
            )
        if shutil.which("proactive-memory-service"):
            return "proactive-memory-service"
        ...
        return f"{shlex.quote(sys.executable)} -m proactive_memory_service"
    ...
    def show_suggestion(self, event: SuggestionReady) -> None:
        ...
        print("\n╭─ Proactive Memory 建议 ─────────────────────────────", file=self.output)
    ...
    def resume_session(self, event: SessionResumeRequested) -> None:
        ...
        environment["PROACTIVE_MEMORY_SOURCE_SUGGESTION_ID"] = event.suggestion_id
        ...
        print(
            f"\n[Proactive Memory] 已授权，正在可见续跑 Hermes Session "
    ...
    def _watch_process(
        ...
        if return_code == 0:
            print(
                f"[Proactive Memory] Hermes 续跑完成：{suggestion_id}",
                file=self.output,
                flush=True,
            )
            return
    ...
    def _report_failure(self, suggestion_id: str, reason: str) -> None:
        print(
            f"[Proactive Memory] 续跑失败：{reason}",
            file=self.output,
            flush=True,
        )
