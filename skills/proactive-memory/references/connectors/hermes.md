...
当前安装说明对应 `0.1.2`，固定下载入口见通用流程。安装器支持 Hermes 基线
...
hermes --help
node --version
proactive-memory-service setup --help
proactive-memory-service doctor --help
```
...
- `PMS_HERMES_ROOT`：当前可执行文件实际使用、且用户允许修改的 Hermes 源码根目录，内含 `ui-tui/`。
- `HERMES_HOME`：该实例的配置目录，默认是用户 Home 下的 `.hermes/`。安装、启动、检查和卸载必须一致；需要时显式使用 `--hermes-home`。

先按系统说明设置 `PMS_HERMES_ROOT`，不能把配置目录、任意源码副本或本服务目录当成实际运行的 Hermes。
...
TUI 备份与指纹记录位于源码下 `ui-tui/.proactive-memory-bridge/`，恢复后仍保留备份。
...
```text
proactive-memory-service setup --harness hermes --web-only --observer-only
```
...
自定义服务地址时，先按系统说明设置 `PROACTIVE_MEMORY_SERVICE_URL`，再从同一终端启动。
...
取得该窗口的实际 Session ID，按系统说明设置 `PMS_SESSION_ID` 后检查：
...
`proactive-memory-tui` 配置。发现时报告残留；需要进一步清理时只处理该组件对应项，
