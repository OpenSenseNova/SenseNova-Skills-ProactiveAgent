...
```powershell
$PMS_PYTHON = py -c "import sys; print(sys.executable)"
if ($LASTEXITCODE -ne 0 -or -not $PMS_PYTHON) { throw "Cannot resolve Python" }
& $PMS_PYTHON -c "import sys; print(sys.version.split()[0], sys.executable); sys.exit(0 if sys.version_info >= (3, 11) else 'Python 3.11+ required')"
...
```powershell
$PMS_PACKAGE = "C:\path\to\proactive_memory_service-0.1.2-py3-none-any.whl"
if (-not (Test-Path -LiteralPath $PMS_PACKAGE -PathType Leaf)) { throw "Package not found" }
$PMS_HERMES_ROOT = "C:\path\to\hermes-agent"
if (-not (Test-Path -LiteralPath (Join-Path $PMS_HERMES_ROOT "ui-tui") -PathType Container)) { throw "Hermes source not found" }
```

在线验收时取得目标窗口的真实 Session ID，再设置变量，不能使用示例 ID 验收：

```powershell
$PMS_SESSION_ID = "actual-session-id"
```

默认数据位置是当前 Windows 用户 Home 下的 `.proactive-memory`，通常为
`%USERPROFILE%\.proactive-memory`。自定义服务地址时，PowerShell 使用以下写法，
将示例值替换为实际服务地址：

```powershell
$env:PROACTIVE_MEMORY_SERVICE_URL = "http://127.0.0.1:8080"
```
...
```bash
PMS_PYTHON="$(python3 -c 'import sys; print(sys.executable)')"
"$PMS_PYTHON" -c 'import sys; print(sys.version.split()[0], sys.executable); sys.exit(0 if sys.version_info >= (3, 11) else "Python 3.11+ required")'
```
...
test -f "$PMS_PACKAGE"
PMS_HERMES_ROOT="/path/to/hermes-agent"
test -d "$PMS_HERMES_ROOT/ui-tui"
```
...
默认数据保存在 WSL 用户的 `~/.proactive-memory/`，不是 Windows 用户目录。
