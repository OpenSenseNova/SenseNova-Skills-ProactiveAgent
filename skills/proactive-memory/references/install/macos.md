...
默认数据目录为 `~/.proactive-memory/`。不需要预先创建 Project 或 Item。
用户要求改位置时，通过服务的 `--data-root` 或 `PROACTIVE_MEMORY_DATA_ROOT` 指定，
不得覆盖原数据目录。

使用默认地址时，无需额外配置。自定义了服务地址时，在启动 Hermes 的同一个终端中设置
`PROACTIVE_MEMORY_SERVICE_URL` 为实际地址。以下仅展示 POSIX Shell 的写法：

```bash
export PROACTIVE_MEMORY_SERVICE_URL="http://127.0.0.1:8080"
```
