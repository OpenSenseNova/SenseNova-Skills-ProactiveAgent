...
- **Skill**：由 Harness 的 Skill 管理机制安装完整的 `proactive-memory/` 目录，必须包含 `SKILL.md` 和 `references/`，不能只复制入口文件。
...
按相应系统文档准备依赖并设置 `PMS_PYTHON`，再继续第 2 步；之后的执行顺序由本文件统一组织。
...
```text
proactive-memory-service doctor --url http://127.0.0.1:8080 --json
```
...
默认保留 `.proactive-memory/` 的全部用户数据；只有用户明确要求删除数据时，才另行确认目标后处理。
