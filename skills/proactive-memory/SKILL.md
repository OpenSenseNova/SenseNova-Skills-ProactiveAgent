...
description: 安装、启动和使用 Proactive Memory，记录项目进展并在合适时机提醒下一步。
---
...
Hermes 的命令。当前 `0.1.2` 候选版可安装指定 Hermes 基线的 Web-only 续跑桥，
会检查源码兼容性、备份并构建；不覆盖未知版本或本地改动。该版本尚未发布到包索引，
也不把构建通过等同于真实接入验收。Windows 说明是安装参考，不代表完整接入已验收。
...
数据默认放在当前用户 Home 下的 `.proactive-memory/`，不是安装包或源码目录。
可用 `--data-root` 或 `PROACTIVE_MEMORY_DATA_ROOT` 覆盖。首次有明确长期目标的
